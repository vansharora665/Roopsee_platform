...
function valuesForStatus(includeProductCatalogInPrompt: boolean) {
  return includeProductCatalogInPrompt
    ? "Prompt generated with inherited quiz_results answers, gender, current front/left/right image URLs, and the imported product catalog for direct GPT product selection."
    : "Prompt generated with inherited quiz_results answers, gender, and current front/left/right image URLs. Product selection will fall back to ingredient-based catalog matching.";
}
...
export function NewReportForm({ initialProfiles }: { initialProfiles: SyncedProfileDto[] }) {
  ...
  function applyProfile(profileId: string) {
    ...
    setStatusMessage(
      "Profile details, quiz_results answers, gender, and front/left/right image URLs were inherited automatically."
    );
  }
          ...
          {selectedProfile ? (
            <div className="space-y-4 rounded-3xl border border-slate-200 bg-slate-50 p-5">
              ...
              <div className="rounded-2xl bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-800">
                <p>
                  Quiz responses, gender, and image URLs were inherited from <code>quiz_results</code> and will be included automatically when you generate the prompt.
                </p>
        ...
        <Card className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm text-slate-600">
                The prompt automatically includes the selected profile metadata, the questionnaire inherited from <code>quiz_results.answers</code>, the scan context, and the current front/left/right image references. If catalog attachment is enabled, it also includes the imported product list so GPT can choose exact products directly.
              </p>
            </div>
