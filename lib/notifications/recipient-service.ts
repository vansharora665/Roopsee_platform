...
async function fetchAppUsersByEmail(emails: string[]) {
  ...
  if (error) {
    console.error("Failed to load app users for notifications:", error.message);
    return new Map();
  }
...
async function fetchPushTokenCounts(userIds: string[]) {
  ...
  if (error) {
    console.error("Failed to load push token counts:", error.message);
    return new Map();
  }
  ...
  for (const profile of profiles) {
    ...
    const key = appUser?.id
      ...
      : normalizedEmail
        ...
        : `profile:${profile.id}`;
    const tokenCount = appUser?.id ? pushTokenCounts.get(appUser.id) || 0 : 0;
    const lastActivity = [
    ...
    const candidate: NotificationRecipient = {
      key,
...
export async function getNotificationRecipientsByKeys(keys: string[]) {
