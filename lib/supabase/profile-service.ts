...
async function upsertNormalizedProfile(row: Record<string, unknown>) {
  ...
  return prisma.syncedProfile.upsert({
    ...
    update: {
      ...
      quizJson: normalized.quizJson
        ? (normalized.quizJson as Prisma.InputJsonValue)
        : undefined,
      quizSummaryJson: normalized.quizSummaryJson as Prisma.InputJsonValue,
      scansJson: normalized.scansJson
        ? (normalized.scansJson as Prisma.InputJsonValue)
        : undefined,
...
async function getLatestSyncedSourceUpdatedAt() {
  ...
  return record?.sourceUpdatedAt ?? null;
}
...
export async function syncSupabaseProfiles(limit = 50) {
  ...
  const latestSyncedSourceUpdatedAt = await getLatestSyncedSourceUpdatedAt();
  ...
  if (latestSyncedSourceUpdatedAt) {
    query = query.gt(
      updatedAtColumn,
      latestSyncedSourceUpdatedAt.toISOString()
    );
  }
...
export function extractStoredScanUrls(
  profile:
    | Pick<SyncedProfileDto, "scanUrls" | "scansJson">
    | null
    | undefined
) {
  if (!profile) {
